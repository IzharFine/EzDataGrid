...
export const FilteredLabelToActions = props => {
    ...
    const [operator, setOperator] = useState(eFilterOperator[Object.keys(eFilterOperator)[0]]);

    const onFilterActionInputChanged = e => {
        let operatorHandler = getOperatorHandler(operator);
        props.onFilterChanged(e.target.value, props.filterText, operatorHandler, operator);
    ...
    const onFilterActionSelectChanged = e => {
        ...
        if(value != null){
            ...
            props.onFilterChanged(value, props.filterText, operatorHandler, operatorText);
        }
    ...
    return(
        ...
        <FilteredValue key={props.filterText} onClick={()=>setIsClicked(!isClicked)} haveFilter={value} >
            {props.filterText}
            <RemoveFilterButton onClick={onCloseButtonClick}>
                X
            </RemoveFilterButton>
        </FilteredValue>
        :
