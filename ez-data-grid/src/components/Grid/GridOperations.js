...
export const adaptRowsData = (data, childrens) => {
  ...
  data.forEach((rowData, rowIndex) => {
    ...
    childrens.forEach((column, columnIndex) => {
      let columnData = new ColumnData(
        column.props.title,
        rowData[column.key],
        columnIndex,
        column.props.isWithoutData
      );
...
export const onFilterChanged = (
  ...
  if (inputValue) {
    rows.forEach((row) => {
      ...
      if (filterIndex !== -1) row.Filtered.splice(filterIndex, 1);
  ...
  } else rowsToShow = removeFilter(columnId, rows);
...
export const onSearchChange = (element, rows) => {
  ...
  rows.forEach((row) => {
    ...
    Object.keys(row.Columns).forEach((columnId) => {
      if (
        !row.Columns[columnId].IsWithoutData &&
        row.Columns[columnId].Value.toString()
          .toLowerCase()
...
export const sortByColumn = (columnId, isAsc, rowsToShow) => {
  ...
  if (isAsc)
    return rowsToSort.sort((a, b) =>
      a.Columns[columnId].Value > b.Columns[columnId].Value ? -1 : 1
    );
...
export const renderRows = (
  ...
  let endingRow =
    startingRow + rowsInPage >= rowsToShow.length
      ? rowsToShow.length
      : startingRow + rowsInPage;
...
export const calculateRowMinWidth = (childrens, disableChooseRows) => {
  ...
  let rowLength = childrens.map((children) => {
    return children.props.minWidth
      ? children.props.minWidth.split("px")[0] * 1 + 30
      : 110;
  });
  rowLength = rowLength.reduce((prev, current) => prev + current, 0);

  // if disableChooseRows add his total width too.
  if (!disableChooseRows) rowLength += 46;
